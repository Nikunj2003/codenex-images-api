...
export async function removeWhiteBorders(
  ...
  try {
    ...
    if (!metadata.width || !metadata.height) {
      logger.warn('Could not get image dimensions for border removal');
      return base64Image;
    }
    ...
    if (borderPercentage < 1) {
      logger.info(`Minimal borders detected (${borderPercentage.toFixed(2)}%), keeping original`);
      return base64Image;
    }
    ...
    logger.info(`Removing borders: cropping from ${width}x${height} to ${cropWidth}x${cropHeight}`);
    logger.info(`Border removed: top=${minY}px, bottom=${height-maxY-1}px, left=${minX}px, right=${width-maxX-1}px`);
    ...
    logger.error('Error removing white borders:', error);
...
export async function ensureDimensions(
  ...
  try {
    ...
    logger.error('Error ensuring dimensions:', error);
    return base64Image;
  }
}